...
pub mod abc {
    ...
    impl Contract {
        ...
        pub fn swap(&mut self, amount_in: u128, index_in: u32) -> Result<(), Error> {
            ...
            let balance_in_before = self.pool_balances[index_in as usize];
            let balance_out_before = self.pool_balances[index_out as usize];
            let amount_out = balance_out_before.saturating_sub(
                balance_in_before
                    .checked_mul(balance_out_before)
                    .ok_or(Error::Math)?
                    .checked_div(balance_in_before.checked_add(amount_in).ok_or(Error::Math)?)
                    .ok_or(Error::Math)?,
            );
            self.increase_balance(caller, index_out, amount_out)?;
            self.pool_balances[index_in as usize] = balance_in_before.checked_add(amount_in).ok_or(Error::Math)?;
            self.pool_balances[index_out as usize] = balance_out_before.checked_sub(amount_out).ok_or(Error::Math)?;
